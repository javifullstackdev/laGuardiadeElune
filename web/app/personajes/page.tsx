...
export default async function PersonajesPage({
  ...
  // Obtener personajes ya registrados en nuestra BD
  ...
  // Obtener personajes de Blizzard (puede fallar si no tiene BNet conectado)
  ...
  // Personajes de Blizzard que aún NO están en nuestra BD
  ...
  return (
    <main className="max-w-3xl mx-auto py-12 px-4">
      <h1 className="text-2xl font-bold mb-8">⚔️ Mis Personajes</h1>
        ...
        {myCharacters.length === 0 ? (
          ...
          <ul className="space-y-2">
            {myCharacters.map((c) => (
              ...
              >
                {c.is_main && <span title="Main">⭐</span>}
                {c.is_verified && <span title="Verificado por Blizzard">🛡️</span>}
        ...
        {bnetError ? (
          ...
          <ul className="space-y-2">
            {unregistered.map((c) => (
              ...
              >
                <span>{FACTION_ICON[c.faction] ?? "⚔️"}</span>
                ...
                {/* Formulario HTML puro — no necesita JS */}
