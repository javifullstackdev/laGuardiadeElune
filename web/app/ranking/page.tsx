// Página pública de ranking — no requiere autenticación
...
const PODIUM_STYLE = [
  { border: "border-yellow-400", glow: "#FFD70033", label: "🥇", size: "text-5xl" },
  { border: "border-gray-400",   glow: "#C0C0C033", label: "🥈", size: "text-4xl" },
  { border: "border-amber-600",  glow: "#CD7F3233", label: "🥉", size: "text-4xl" },
];
...
export default async function RankingPage() {
  const res = await fetch("http://localhost:8000/users/ranking?limit=20", {
    cache: "no-store",
  });
      ...
      <div className="max-w-3xl mx-auto px-4 pb-16">
        ...
        {top3.length > 0 && (
          <div className="flex items-end justify-center gap-4 mb-12">
            {/* Reordenar visualmente: 2º - 1º - 3º */}
            {[top3[1], top3[0], top3[2]].filter(Boolean).map((entry, i) => {
              const visualPos = [1, 0, 2][i]; // índice original
              ...
              const icon  = CLASS_ICON[charClass ?? ""]  ?? "🧙";
                ...
                >
                  ...
                  {entry.avatar_url ? (
                    <img
                      ...
                      alt={entry.username}
                      className="w-14 h-14 rounded-full border-2 border-gray-600 my-2"
                    />
                  ) : (
                    <div className="w-14 h-14 rounded-full bg-gray-700 flex items-center justify-center text-2xl my-2">
                      {icon}
                    </div>
                  )}
                  ...
                  {entry.character && (
                    <p
                      ...
                      style={{ color }}
                    >
                      {icon} {entry.character.name}
                      {entry.character.is_verified && " 🛡️"}
                    </p>
                  )}
          ...
          <ul className="space-y-2">
            {rest.map((entry) => {
              ...
              return (
                <li
                  key={entry.discord_id}
                  className="flex items-center gap-4 px-4 py-3 rounded-xl bg-gray-900 hover:bg-gray-800 transition-colors"
                >
                  {/* Posición */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {entry.character ? (
                      <p className="text-xs truncate" style={{ color }}>
                        {icon} {entry.character.name}-{entry.character.realm}
                        {className && ` · ${className}`}
                        {entry.character.is_verified && " 🛡️"}
                      </p>
                    ) : (
